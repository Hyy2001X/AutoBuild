...
Sources_Download() {
	if [[ -f ${Home}/Projects/${Project}/Makefile ]];then
		...
		case ${Choose}_Branch in
		q)
			break
		;;
		*)
			if [[ ${Choose}_Branch -le $Max_Branch_Line ]] 2>/dev/null ;then
				if [[ ! ${Choose}_Branch == 0 ]];then
					clear
					Branch_Line=$(expr ${Choose}_Branch + 1)
...
Dir_Check() {
	cd ${Home}
	for WD in $(cat ./Additional/Dir.list)
	do
		[ ! -d ./$WD ] && mkdir -p $WD
	done
...
Dependency="$(cat ${Home}/Additional/Depends.list)"
